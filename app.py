    ...
    if not stats_df.empty:
        ...
        if player_col:
            ...
            if st.button("Generar Radar"):
                ...
                if min_col:
                    stats_df[min_col] = pd.to_numeric(stats_df[min_col].astype(str).str.replace(",", ""), errors="coerce").fillna(0)
                ...
                stats_a = [float(p_a[f"{m}_pct"].values[0]) if not p_a.empty else 0 for m in metrics]
                stats_b = [float(p_b[f"{m}_pct"].values[0]) if not p_b.empty else 0 for m in metrics]
